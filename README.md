...
>
> **这个仓库里有什么 / 没有什么**：只有**代码、文档与图**；
> **不含任何抓取到的数据**（`data/` 被 gitignore，约 270 MB）—— 按下面「复跑」一节逐条跑即可重建，每步留取数账。
...
🪜 **两级名单（会亏 → 会戴帽）** → [`docs/两级名单-亏损与戴帽.md`](docs/两级名单-亏损与戴帽.md)：P(戴帽|亏损) **4.70%** vs P(戴帽|没亏) **0.33%**（14 倍）；
